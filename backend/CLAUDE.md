...
- **Views**: `validators/views.py`
  ...
  - `/api/v1/validators/wallets/grafana/` - Public minimal roster for the Grafana Infinity datasource (`GrafanaValidatorSerializer`). Flat array, one row per wallet across ALL statuses; fields: `network` (Grafana label value e.g. `asimov-phase5`), `node` (on-chain validator address == Prometheus `genlayer_node_info` `node` label, lowercased), `name`, `status`, `operator`, `account`/`account_name` (only for visible operators), `explorer_url`, `identity_missing` (setup-completeness flag: empty string when correctly set up, else comma-joined subset of `moniker`/`logo`/`description` — blank `getIdentity()` fields — and `portal` — wallet not linked to any portal account; reveals only that a link exists, never who, so it's safe for non-visible operators. Feeds the Grafana Wall of Shame "Identity" column). Excludes observability/shame fields by design. Cached 60s. Optional `?network=asimov|bradbury` filter.
