/**
 * Content-link resolution for the history-based portal router.
 *
 * Links managed through the admin (hero banners, GenNews announcements) are
 * stored in a URLField, so links that point back into the portal itself arrive
 * as absolute URLs. Treating those as external opens the portal in a new tab,
 * where the browser back button has no history to return to. This helper
 * classifies a raw link and rewrites same-origin URLs into in-app path routes
 * so they navigate within the SPA and keep browser history intact. It also
 * normalizes legacy hash routes (`#/mission/7`,
 * `https://portal.genlayer.foundation/#/mission/7`) into clean paths so older
 * content authored before the hash-routing removal still resolves.
 */
...
export function resolvePortalLink(
  ...
  if (origin && url.origin === origin) {
    if (url.hash.startsWith('#/')) return { href: url.hash.slice(1), external: false };
