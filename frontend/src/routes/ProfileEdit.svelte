<script>
  ...
  async function startCreatorJourney() {
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        sessionStorage.setItem('journeySuccess', 'You are now a Community Member! Start growing the community through referrals.');
        loadUserData();
      }
    } catch (err) {
  ...
  async function handleSocialLinked(updatedUser) {
    // Update local user state with the updated social connection info
    user = updatedUser;
  }
</script>
...
<div class="container mx-auto px-4 py-8">
  ...
  {#if user}
    ...
    <div class="bg-white shadow rounded-lg p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          {#if user.github_connection?.username}
            <div class="flex items-center gap-2">
              <div class="flex-1 px-3 py-2 bg-gray-50 border border-gray-300 rounded-md text-gray-700 flex items-center gap-2">
                ...
                {user.github_connection.username}
              </div>
              <a
                href="https://github.com/{user.github_connection.username}"
                target="_blank"
            ...
            {#if user.github_connection?.linked_at}
              <p class="text-xs text-gray-500 mt-1">Linked on {new Date(user.github_connection.linked_at).toLocaleDateString()}</p>
            {/if}
          {:else}
            <GitHubLink
        ...
        <!-- Twitter/X -->
        <div>
          <label for="twitter_oauth" class="block text-sm font-medium text-gray-700 mb-2">Twitter/X (OAuth)</label>
          {#if user.twitter_connection?.username}
            <div class="flex items-center gap-2">
              ...
              <a
                href="https://twitter.com/{user.twitter_connection.username}"
                target="_blank"
                rel="noopener noreferrer"
                class="px-3 py-2 text-gray-600 hover:text-gray-800 border border-gray-300 rounded-md flex items-center gap-1"
            ...
            {#if user.twitter_connection?.linked_at}
              <p class="text-xs text-gray-500 mt-1">Linked on {new Date(user.twitter_connection.linked_at).toLocaleDateString()}</p>
            {/if}
          {:else}
            <TwitterLink
              onLinked={handleSocialLinked}
            />
            <p class="text-xs text-gray-500 mt-1">Link your Twitter for verified social presence</p>
          {/if}
        </div>

        <!-- Discord (OAuth) -->
        <div>
          <label for="discord_oauth" class="block text-sm font-medium text-gray-700 mb-2">Discord (OAuth)</label>
          {#if user.discord_connection?.username}
            ...
            {#if user.discord_connection?.linked_at}
              <p class="text-xs text-gray-500 mt-1">Linked on {new Date(user.discord_connection.linked_at).toLocaleDateString()}</p>
            {/if}
          {:else}
            <DiscordLink
              onLinked={handleSocialLinked}
            />
            <p class="text-xs text-gray-500 mt-1">Link your Discord for verified community membership</p>
          {/if}
        </div>
      </div>
    </div>
    ...
    <!-- Profile Sections -->
      ...
      {@const needsCommunity = !user.creator}
      {@const inactiveCount = [needsBuilder, needsValidator, needsSteward, needsCommunity].filter(Boolean).length}
      ...
      <!-- Steward and Community Grid -->
      {#if needsSteward || needsCommunity}
        {@const bothStewardCommunity = needsSteward && needsCommunity}
        <div class="{bothStewardCommunity ? 'grid grid-cols-1 md:grid-cols-2 gap-4 mt-6' : 'mt-6'}">
          <!-- Inactive Steward -->
          {#if needsSteward}
      ...
      <!-- Active Community Profile Card (full width when user is already a community member) -->
      {#if user.creator}
        <div class="bg-purple-100 shadow rounded-lg p-6 border border-purple-300 mt-6">
          ...
          <p class="text-sm text-purple-800 mb-2">You're a Community Member! Focus on growing the community through referrals.</p>
