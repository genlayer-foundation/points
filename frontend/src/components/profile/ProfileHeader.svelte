...
>
  ...
  <div class="p-[12px] bg-white rounded-bl-[8px] rounded-br-[8px]">
    ...
    <div class="flex items-center justify-between w-full mt-6">
      <div class="flex gap-[8px] items-center text-[14px]">
        {#if participant?.github_username}
          <a
            href={`https://github.com/${participant.github_username}`}
            target="_blank"
            rel="noopener noreferrer"
            class="flex gap-[4px] items-center justify-center p-[4px] rounded-[4px] bg-white border border-[#f5f5f5] hover:bg-gray-50 transition-colors"
          >
            <svg
              class="w-4 h-4 opacity-80 flex-shrink-0"
              viewBox="0 0 24 24"
              fill="currentColor"
              ><path
              ...
              /></svg
            >
            <span class="font-medium text-black tracking-[0.28px]"
              >{participant.github_username}</span
            >
          </a>
        {/if}
        {#if participant?.twitter_handle}
          ...
          >
            ...
            >
            <span class="font-medium text-black tracking-[0.28px]"
              >{participant.twitter_handle}</span
            >
          </a>
        {/if}
        {#if participant?.discord_handle}
          <div
            class="flex gap-[4px] items-center justify-center p-[4px] rounded-[4px] bg-white border border-[#f5f5f5]"
          >
            <svg
              class="w-4 h-4 opacity-80 flex-shrink-0"
              viewBox="0 0 24 24"
            ...
            >
            <span class="font-medium text-black tracking-[0.28px]"
              >{participant.discord_handle}</span
            >
          </div>
        {/if}
      </div>
