<script>
  ...
  let requiredEvidenceSlot = $state({
    description: "",
    url: "",
    selectedType: null,
    error: "",
    ...
    try {
      ...
      for (const ct of allTypes) {
        for (const ut of ct.accepted_evidence_url_types || []) {
          if (!urlTypeMap.has(ut?.slug)) urlTypeMap.set(t?.slug, ut);
        }
      }
        ...
        try {
          ...
          if (mission) {
            const parentType = types.find(
              (t) => t.id === mission.contribution_type,
            );
            if (parentType) {
  ...
  let missingSocialAccounts = $derived.by(() => {
    ...
    const user = $userStore.user;
    if (!user)
      return selectedType.required_social_accounts.map(
        (a) => socialAccountLabels[a] || a,
      );
  ...
  let hasEvidencePatternMismatch = $derived.by(() => {
    return evidenceSlots.some(
      (slot) =>
        slot.url &&
        slot.selectedType &&
        !slot.selectedType.is_generic &&
        !urlMatchesType(slot.url, slot.selectedType),
    );
  });
  ...
  let filteredItems = $derived.by(() => {
    ...
    if (searchQuery) {
      ...
      matchingTypes = categoryTypes.filter(
        (t) =>
          matchingTypes.includes(t) || typeIdsWithMatchingMissions.has(t.id),
      );
    }
  ...
  function handleEvidenceTypeChange(index, slug) {
    ...
    if (urlType) {
      evidenceSlots[index].description = urlType.is_generic
        ? "Other"
        : urlType.name;
    }
  }
  ...
  <form onsubmit={handleSubmit} class="w-full flex flex-col gap-[20px]">
    ...
    >
              ...
              {:else}
                {#each filteredItems as item}
                  <button
                    ...
                    class="w-full text-left flex items-start flex-col p-[12px] hover:bg-gray-50 border-b border-[#f5f5f5] last:border-0 {(item.itemType ===
                      'type' &&
                      selectedType?.id === item.data.id &&
                      !selectedMission) ||
                      ...
                      <div class="mt-1 flex items-center gap-1">
                        <span
                          ...
                          >{#if item.data.current_multiplier}{Math.round(
                              item.data.min_points *
                                item.data.current_multiplier,
                            )} - {Math.round(
                              item.data.max_points *
                                item.data.current_multiplier,
      ...
      {#if selectedType && !showTypeDropdown}
        ...
        >
          ...
          {:else}
            <span class="font-['Switzer'] font-semibold text-[14px] text-black"
            ...
            {/if}
            <p
              class="font-['Switzer'] text-[12px] text-green-600 font-medium mt-1"
            >
    ...
    {#if selectedType && !canShowFormDetails}
      ...
      >
        ...
        >
          Link your {gateRequiredSocialAccounts.join(", ")} account{gateRequiredSocialAccounts.length >
          1
            ? "s"
            : ""} from your profile to submit this contribution.
        </p>
        <a
          href="#/profile"
    ...
    {#if canShowFormDetails}
      ...
      >
        ...
        <div class="w-full flex flex-col gap-[12px] mt-2">
          <label
            ...
            class="font-['Switzer'] font-semibold leading-[25px] text-[20px] text-black tracking-[0.4px]"
          >
            Title <span class="text-[14px] font-normal text-[#ababab]"
              >(optional)</span
            >
          </label>
          <div
      ...
      >
                ...
                {#if acceptedEvidenceTypes.length > 0}
                  <div class="flex items-center gap-2 pr-[30px]">
                    {#if slot.selectedType}
                      <span
                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[12px] font-['Switzer'] font-medium bg-[#f0f0f0] text-[#1a1c1d]"
                      >
                        <svg
                    ...
                    {#if acceptedEvidenceTypes.length > 1}
                      <select
                        ...
                        onchange={(e) =>
                          handleEvidenceTypeChange(index, e.target.value)}
                  ...
                  {#if slot.url && slot.selectedType && !slot.selectedType.is_generic && !urlMatchesType(slot.url, slot.selectedType)}
                    <p
                      class="text-[12px] text-red-500 font-['Switzer'] pl-1 -mt-1"
                    >
                      This URL doesn't match the expected format for {slot
                        .selectedType.name}.
                    </p>
                  {/if}
                {/if}
        ...
        {#if evidenceRequiredAccounts.length > 0}
          <div
            class="w-full bg-[#fafafa] border border-[#e0e0e0] rounded-[12px] p-[16px] flex flex-col gap-[6px] mt-2"
          >
            <p
              class="font-['Switzer'] font-medium text-[14px] text-black tracking-[0.28px]"
            >
              Link your {evidenceRequiredAccounts
                .map((a) => socialAccountLabels[a] || a)
                .join(", ")} account{evidenceRequiredAccounts.length > 1
                ? "s"
                : ""} from your profile to verify evidence ownership.
            </p>
            <a
              href="#/profile"
              class="font-['Switzer'] text-[13px] text-[#6b6b6b] hover:text-black transition-colors tracking-[0.26px]"
      ...
      <div class="flex gap-[8px] items-center mt-2 pb-[60px]">
        <button
          type="submit"
          disabled={submitting ||
            !canSubmitCurrentCategory ||
            evidenceRequiredAccounts.length > 0 ||
            hasEvidencePatternMismatch}
