<div class="flex flex-col md:flex-row gap-[20px] md:gap-[24px] items-stretch md:items-center p-[12px] sm:p-[16px] md:p-[20px] rounded-[8px]">
  <!-- Left: text -->
  <div class="md:flex-1 flex flex-col gap-[16px] md:gap-[24px] justify-center">
    <h2 class="text-[28px] md:text-[32px] font-medium font-display leading-[34px] md:leading-[40px] text-black" style="letter-spacing: -0.02em;">
      There's a place for everyone
    </h2>
    <p class="text-[15px] md:text-[17px] text-black leading-[23px] md:leading-[28px]" style="letter-spacing: 0.02em;">
      GenLayer is built by its ecosystem. Whether you code, run infrastructure, or create content — your contributions move GenLayer forward and are rewarded accordingly.
    </p>
    <div class="flex flex-col gap-[10px] md:gap-[4px]">
      <div class="flex gap-[10px] items-start md:items-center">
        <img src="/assets/icons/checkbox-circle-fill.svg" alt="" class="w-4 h-4 flex-shrink-0" />
        <span class="text-[15px] md:text-[17px] text-black leading-[23px] md:leading-[28px]" style="letter-spacing: 0.02em;">Sign up as a Builder and deploy your first Intelligent Contract</span>
      </div>
      <div class="flex gap-[10px] items-start md:items-center">
        <img src="/assets/icons/checkbox-circle-fill.svg" alt="" class="w-4 h-4 flex-shrink-0" />
        <span class="text-[15px] md:text-[17px] text-black leading-[23px] md:leading-[28px]" style="letter-spacing: 0.02em;">Join the Validator waitlist and help secure the network</span>
      </div>
      <div class="flex gap-[10px] items-start md:items-center">
        <img src="/assets/icons/checkbox-circle-fill.svg" alt="" class="w-4 h-4 flex-shrink-0" />
        <span class="text-[15px] md:text-[17px] text-black leading-[23px] md:leading-[28px]" style="letter-spacing: 0.02em;">Join the Community to create content and participate in events</span>
      </div>
    </div>
  </div>

  <!-- Right: illustration -->
  <div class="md:flex-1 bg-white border border-[#f5f5f5] rounded-[8px] h-[260px] sm:h-[320px] md:h-[400px] overflow-hidden relative flex items-center justify-center p-3 md:p-4">
    <img
      src="/assets/illustrations/journey-illustration.svg"
      alt="Journey illustration"
      class="w-full h-full max-w-[500px] object-contain"
    />
  </div>
</div>
