  ...
  <div class="bg-white px-4 py-3 border-t border-gray-200">
    <div class="flex justify-between items-center">
      <div class="text-sm text-gray-500">
        {m.pag_showing_items({ from: Math.min((page - 1) * limit + 1, totalCount), to: Math.min(page * limit, totalCount), total: totalCount })}
      </div>
      <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label={m.pag_pagination()}>
        <button
