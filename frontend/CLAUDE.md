...
The portal UI is multilingual: **en (source), es, ko, zh-Hans, tr**, via [Paraglide JS 2.x](https://paraglidejs.com).

- **Messages** live in `messages/{locale}.json` (flat key → string). Adding a UI string means adding the key to **all five** files; missing keys fall back to English.
- **Compiled output** `src/lib/paraglide/` is **gitignored** — generated by `paraglideVitePlugin` (configured in BOTH `vite.config.js` and `vitest.config.js`; inlang project config in `project.inlang/settings.json`).
- **Usage in components**: `import { m } from '../lib/paraglide/messages.js'` then `{m.nav_overview()}`. Never hardcode user-facing English strings in translated surfaces.
- **Wrapper** `src/lib/i18n.js`: `SUPPORTED_LOCALES` (display names), `setPortalLocale(code)`, `ogLocale()`, `dateFnsLocale()` (pass to date-fns `format`/`formatDistanceToNow` calls), `formatNumber`/`formatCompactNumber`/`formatDate` (Intl-based).
- **Locale resolution**: localStorage `PARAGLIDE_LOCALE` → browser language → `en`. Switching locale does a **full page reload** (deliberate: module-scope copy constants like `resources.js`, `socialTaskLabels.js`, journey configs never go stale). `<html lang>` is synced in `main.js`.
- **Selector**: `src/components/LanguageSelector.svelte` (native `<select>` + globe icon) — desktop navbar + mobile sidebar bottom area; reachable logged-out.
- **Metadata**: `src/lib/routeMeta.js` MUST stay plain English strings — `scripts/generate-og-pages.mjs` imports it in Node at build time (static OG pages are English by design). Localized overrides go in `LOCALIZED_ROUTE_META` in `src/lib/meta.js`; `og:locale` and JSON-LD `inLanguage` follow the active locale at runtime.
- **Out of scope (v1)**: backend/API-authored content (missions, hero banners, POAPs, notifications, social tasks), Foundations/Genesis doc bodies, and Terms/Privacy body text stay English.
- **Tests**: `src/tests/i18n.test.js`. In jsdom use `setLocale(code, { reload: false })` — jsdom can't navigate.
